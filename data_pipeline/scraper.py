...
_adapter = HTTPAdapter(
    max_retries=_retry_strategy, pool_connections=100, pool_maxsize=100
)
...
def adjust_rate_limit_if_needed():
    ...
    with _throttle_lock:
        if (
            not ADAPTIVE_THROTTLE_ENABLED
            or _total_request_attempts < THROTTLE_CHECK_INTERVAL
        ):
            return
        ...
        if ratio_429 >= THROTTLE_429_THRESHOLD:
            ...
            REQUESTS_PER_SECOND = max(
                THROTTLE_MIN_RPS, REQUESTS_PER_SECOND * THROTTLE_REDUCTION_FACTOR
            )
...
def rate_limited_fetch_page(url, params, max_retries=3, timeout=30):
    ...
    with _rate_limit_lock:
        ...
        if _request_count % RATE_LIMIT_LOGGING == 0:
            ...
            logging.info(
                f"Effective request rate: {rps:.2f} req/sec over last {elapsed_window:.1f}s"
            )
            _request_count = 0
            _start_window = now
...
def remove_duplicates(table_name, chunk_size=1000, max_removals=MAX_DUPLICATES_REMOVAL):
    ...
    car_id_to_remove = df_full.loc[
        df_full.duplicated(subset=["car_id"], keep="first"), "car_id"
    ].values
    logging.info(
        f"New method has: {len(car_id_to_remove)} duplicate entries in database."
    )
    ...
    if len(car_id_to_remove) > max_removals:
        logging.warning(
            f"More duplicates detected than threshold. Limiting removal to {max_removals}"
        )
...
def fetch_and_insert_postcodes():
    ...
    postcodes_not_in_database = postcodes_in_car_database.difference(
        postcodes_in_database
    )
    postcodes_to_insert = []
    count_added = 0
...
def save_ranges_to_file(
    price_list, km_list, path: str | None = None, settings: dict | None = None
):
    if path is None:
        path = _ranges_file_path()
    data = {
...
def split_and_insert_midpoint(ranges_list: list, start: float, end: float):
    ...
    try:
        ...
        for i in range(len(ranges_list) - 1):
            if float(ranges_list[i]) == float(start) and float(
                ranges_list[i + 1]
            ) == float(end):
...
def process_page(base_url, params, car_ids_in_database, car_ids_in_upsert):
    ...
    for car in car_listings:
        car_id = car.get("id")
        if car_id not in car_ids_in_upsert and (
            (car_id not in car_ids_in_database) or PROCESS_ALL
        ):
            # Mileage
            try:
...
def scrape_km_range(
    ...
    for page_index in range(PAGE_LIMIT):
        ...
        page_results = process_page(
            base_url, page_params, car_ids_in_database, car_ids_in_upsert
        )
        ...
        if page_index + 1 == PAGE_LIMIT:
            logging.info(
                f"Reached page limit for price {page_params['pricefrom']}-{page_params['priceto']} "
...
def scrape_cars(table_name):
    ...
    car_rows = fetch_all_rows_in_batches(
        table_name, "car_id", "car_id", batch_size=50000
    )
    ...
    for price_index in range(len(price_ranges) - 1):
        ...
        logging.info(
            f"Evaluating price range {price_from}-{price_to} "
                                ...
                                with ranges_lock:
                                    ...
                                    if key not in splits_to_apply:
                                        if is_integer_midpoint(kf, kt):
                                            changed = split_and_insert_midpoint(
                                                km_list, kf, kt
                                            )
                                            if changed:
                                                km_ranges = np.array(km_list)
                                                ...
                                                logging.info(
                                                    f"Auto-split km range {kf}-{kt} applied in-memory (queued save)."
                                                )
                                        else:
    ...
    if AUTO_ADJUST_ON_LIMIT and splits_to_apply:
        try:
            save_ranges_to_file(
                price_list, km_list, ranges_file, settings=ranges_settings
            )
            logging.info(
                f"Saved {len(splits_to_apply)} auto-adjusted ranges to {ranges_file}."
            )
        except Exception as e:
